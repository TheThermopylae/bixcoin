<template>
  <div class="flex justify-between items-center min-h-screen">
    ...
    <div class="m-auto p-2">
      ...
      ></NuxtLink>
      <div class="border rounded-lg w-[350px] md:w-[400px]">
        <div class="flex justify-between border-b p-5">
          <h1 class="text-xl font-extrabold">ورود به حساب</h1>
          <nuxtLink to="/register" class="flex items-center gap-2 text-primary">
            ساخت حساب
            <svg
        ...
        <div class="p-5">
          <div role="tablist" class="tabs tabs-bordered">
            ...
            <div role="tabpanel" class="tab-content">
              <form @submit.prevent="showOtpFunc">
                <input
                  id="email"
                  type="email"
                  placeholder="ایمیل"
                  class="block my-4 bg-main p-3 w-full rounded-lg"
                  v-model="identifier"
                />
                <button
                  class="btn-link-c w-full rounded-lg p-2.5 h-12"
                  v-if="!loading"
                >
                  ارسال کد
                </button>
                <button
                  class="btn-link-c w-full rounded-lg p-2.5 h-12 centered"
                  v-else
                >
            ...
            <div role="tabpanel" class="tab-content">
              <form>
                <input
                  ...
                  class="block mt-4 bg-main p-3 w-full rounded-lg"
                />
                <NuxtLink to="/" class="text-primary block my-4"
                  >فراموشی رمز</NuxtLink
                >
                <button class="w-full btn-link-c rounded-lg p-3">ورود</button>
              </form>
            </div>
          </div>
        </div>
