<template>
  <section
    class="my-20 flex flex-col-reverse gap-5 lg:flex-row justify-between lg:items-center"
  >
    <div>
      <h2 class="font-extrabold">ترید در هر زمان و هر کجا</h2>
      <p class="text-gray-600 my-5">
        موقعیت‌های جدید را فوراً باز کنید، چه در برنامه Bybit یا وب.
      </p>
      <div class="flex gap-5">
        <div
          class="w-[100px] h-[100px] bg-white rounded-lg flex justify-center items-center flex-col"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="2em"
            height="2em"
            viewBox="0 0 24 24"
          >
            <path
              fill="#000"
              d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47c-1.34.03-1.77-.79-3.29-.79c-1.53 0-2 .77-3.27.82c-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51c1.28-.02 2.5.87 3.29.87c.78 0 2.26-1.07 3.81-.91c.65.03 2.47.26 3.64 1.98c-.09.06-2.17 1.28-2.15 3.81c.03 3.02 2.65 4.03 2.68 4.04c-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5c.13 1.17-.34 2.35-1.04 3.19c-.69.85-1.83 1.51-2.95 1.42c-.15-1.15.41-2.35 1.05-3.11"
            />
          </svg>
          اپل استور
        </div>
        <div
          class="w-[100px] h-[100px] bg-white rounded-lg flex justify-center items-center flex-col"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="2em"
            height="2em"
            viewBox="0 0 72 72"
          >
            <path
              fill="#b1cc33"
              fill-rule="evenodd"
              d="M20.653 28.06C11.303 33.396 5 43.462 5 55q0 .502.016 1h61.968q.016-.498.016-1c0-11.537-6.303-21.603-15.653-26.94l4.853-8.406a2 2 0 0 0-3.464-2L47.743 26.3A30.9 30.9 0 0 0 36 24a30.9 30.9 0 0 0-11.743 2.301l-4.993-8.647a2 2 0 1 0-3.464 2z"
              clip-rule="evenodd"
            />
            <path
              d="m20.653 27.06l.496.868a1 1 0 0 0 .37-1.369zM5.016 55l-1 .032a1 1 0 0 0 1 .968zm61.968 0v1a1 1 0 0 0 1-.968zM51.347 27.06l-.866-.5a1 1 0 0 0 .37 1.368zm4.853-8.406l.866.5zm-.732-2.732l.5-.867zm-2.732.732l-.866-.5zM47.743 25.3l-.379.926a1 1 0 0 0 1.245-.426zm-23.486 0l-.866.5a1 1 0 0 0 1.245.426zm-4.993-8.647l-.866.5zm-2.732-.732l-.5-.867zm-.732 2.732l-.866.5zM6 54c0-11.164 6.098-20.905 15.149-26.072l-.992-1.737C10.508 31.7 4 42.089 4 54zm.015.968Q6 54.486 6 54H4q0 .518.016 1.032zM66.985 54H5.014v2h61.97zM66 54q0 .486-.015.968l1.999.064Q68 54.518 68 54zM50.851 27.928C59.902 33.095 66 42.836 66 54h2c0-11.91-6.508-22.3-16.157-27.81zm4.483-9.774l-4.853 8.405l1.732 1l4.853-8.405zm-.366-1.366a1 1 0 0 1 .366 1.366l1.732 1a3 3 0 0 0-1.098-4.098zm-1.366.366a1 1 0 0 1 1.366-.366l1-1.732a3 3 0 0 0-4.098 1.098zM48.609 25.8l4.993-8.647l-1.732-1l-4.993 8.647zM36 24c4.024 0 7.86.792 11.364 2.227l.758-1.85A31.9 31.9 0 0 0 36 22zm-11.364 2.227A29.9 29.9 0 0 1 36 24v-2a31.9 31.9 0 0 0-12.122 2.376zm.487-1.426l-4.993-8.647l-1.732 1l4.993 8.647zm-4.993-8.647a3 3 0 0 0-4.098-1.098l1 1.732a1 1 0 0 1 1.366.366zm-4.098-1.098a3 3 0 0 0-1.098 4.098l1.732-1a1 1 0 0 1 .366-1.366zm-1.098 4.098l4.853 8.405l1.732-1l-4.853-8.405z"
            />
            <circle cx="22.5" cy="42.5" r="2.5" />
            <circle cx="49.5" cy="42.5" r="2.5" />
          </svg>
          اندروید APK
        </div>
        <div
          class="w-[100px] h-[100px] bg-white rounded-lg flex justify-center items-center flex-col"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="1.81em"
            height="2em"
            viewBox="0 0 256 283"
          >
            <path
              fill="#ea4335"
              d="M119.553 134.916L1.06 259.061a32.14 32.14 0 0 0 47.062 19.071l133.327-75.934z"
            />
            <path
              fill="#fbbc04"
              d="M239.37 113.814L181.715 80.79l-64.898 56.95l65.162 64.28l57.216-32.67a31.345 31.345 0 0 0 0-55.537z"
            />
            <path
              fill="#4285f4"
              d="M1.06 23.487A30.6 30.6 0 0 0 0 31.61v219.327a32.3 32.3 0 0 0 1.06 8.124l122.555-120.966z"
            />
            <path
              fill="#34a853"
              d="m120.436 141.274l61.278-60.483L48.564 4.503A32.85 32.85 0 0 0 32.051 0C17.644-.028 4.978 9.534 1.06 23.399z"
            />
          </svg>
          گوگل پلی
        </div>
      </div>
    </div>
    <img src="/public/AppImage.avif" alt="app-image" class="lg:w-1/2" />
  </section>
</template>
