<template>
  <div class="overflow-x-auto mt-20">
    <table class="w-full min-w-[600px]">
      <thead>
        <tr>
          <th></th>
          <th>تریدر</th>
          <th>PnL هفتگی (USD)</th>
          <th>اقدام</th>
        </tr>
      </thead>
      <tbody>
        <tr class="border-b border-b-gray-800" v-for="item in 5" :key="item">
          <td class="mt-3">1</td>
          <td>
            <div class="flex gap-2 my-3 items-center">
              <img
                class="w-10 h-10"
                src="https://www.bybit.com/leaderboard/images/default_avatar.png"
                alt="profile"
              />
              <div>
                <h4>**sa</h4>
                <div class="flex gap-3 flex-row-reverse">
                  <span class="text-green-500">5W</span>
                  <span class="text-primary">4B</span>
                  <span class="text-blue-500">2M</span>
                </div>
              </div>
            </div>
          </td>
          <td class="text-green-500">879,890.76+</td>
          <td>
            <div class="flex gap-4 items-center">
              <div class="flex gap-1">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                  height="1em"
                  viewBox="0 0 24 24"
                >
                  <g fill="none">
                    <path
                      fill="currentColor"
                      d="m15 10l-.986-.164A1 1 0 0 0 15 11zM4 10V9a1 1 0 0 0-1 1zm16.522 2.392l.98.196zM6 21h11.36v-2H6zM18.56 9H15v2h3.56zm-2.573 1.164l.805-4.835L14.82 5l-.806 4.836zM14.82 3h-.214v2h.214zm-3.543 1.781L8.763 8.555l1.664 1.11l2.516-3.774zM7.93 9H4v2h3.93zM3 10v8h2v-8zm17.302 8.588l1.2-6l-1.96-.392l-1.2 6zM8.762 8.555A1 1 0 0 1 7.93 9v2a3 3 0 0 0 2.496-1.336zm8.03-3.226A2 2 0 0 0 14.82 3v2zM18.56 11a1 1 0 0 1 .981 1.196l1.961.392A3 3 0 0 0 18.561 9zm-1.2 10a3 3 0 0 0 2.942-2.412l-1.96-.392a1 1 0 0 1-.982.804zM14.606 3a4 4 0 0 0-3.328 1.781l1.664 1.11A2 2 0 0 1 14.606 5zM6 19a1 1 0 0 1-1-1H3a3 3 0 0 0 3 3z"
                    />
                    <path stroke="currentColor" stroke-width="2" d="M8 10v10" />
                  </g>
                </svg>
                0
              </div>
              <button
                class="px-4 py-2 border border-primary text-primary rounded hover:bg-primary hover:text-white transition-all"
              >
                دنبال کردن
              </button>
            </div>
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</template>

<style scoped>
th {
  text-align: right;
}
</style>
